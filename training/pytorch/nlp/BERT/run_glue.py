...
def parse_args(parser=argparse.ArgumentParser()):
    ...
    parser.add_argument("--train_batch_size",
                        default=32,
                        type=int,
                        help="Batch size per GPU or GCU for training.")
    parser.add_argument("--eval_batch_size",
                        default=8,
                        type=int,
                        help="Batch size per GPU or GCU for eval.")
            ...
            for step, batch in enumerate(
                ...
                if (step + 1) % args.gradient_accumulation_steps == 0:
                    ...
                    if args.device.lower() == "gpu":
                        ...
                        dm.optimizer_step(optimizer, [loss,logits], safe_async=not args.distributed, model=model)
                    global_step += 1
                if step % args.log_freq == 0 and args.perf == False:

...
def main():
    ...
    tb_writer = SummaryWriter('runs/tensorboard/gcu_{}'.format(args.local_rank if args.local_rank >=0 else 0))
    ...
    if  args.device.lower() == "gpu":
      ...
      from torch_dtu.utils.data.distributed import DistributedSampler
      device = dm.dtu_device(args.local_rank if args.local_rank >=0 else 0)
      if args.distributed:
    ...
    if args.distributed and APEX_IS_AVAILABLE and args.use_apex and args.device.lower() == 'gpu':
        ...
        from torch_dtu.nn.parallel import DistributedDataParallel as DDP
    ...
    if args.do_train:
        ...
        if args.amp:
            if args.device.lower() == "gpu":
                ...
                scaler = torch_dtu.amp.GradScaler(use_zero_grad=True)
                autocast = torch_dtu.amp.autocast
        print("Start training")
        start_time = time.time()
        for epoch in range(int(args.num_train_epochs)):
            ## time will reset in end of traversal epoch
            timers('interval time').reset()
            train_iter = tqdm(train_dataloader, desc="Iteration", disable=args.disable_progress_bar) if is_main_process(dist) else train_dataloader
                ...
                if (step + 1) % args.gradient_accumulation_steps == 0:
                    if args.device.lower() == "gpu":
                        ...
                        if args.amp:
                            scaler.step(optimizer)
                            scaler.update()
                            dm.unlazy(dm.collect_amp_training_params(model, optimizer, scaler)+[loss,start_positions,end_positions])
                        else:
                            dm.optimizer_step(optimizer, [loss,start_positions,end_positions], safe_async=not args.distributed, model=model)
                    global_step += 1
    ...
    if args.do_predict and (args.local_rank == -1 or is_main_process(dist)):
        ...
        for input_ids, input_mask, segment_ids, example_indices in tqdm(eval_dataloader, desc="Evaluating", disable=args.disable_progress_bar):
            ...
            if(args.device.lower() != "gpu"):
                dm.unlazy([batch_start_logits, batch_end_logits])

...
def create_pretraining_dataset(input_file, max_pred_length, shared_list, args, worker_init):
    ...
    train_dataloader = DataLoader(train_data, sampler=train_sampler,
                                  batch_size=args.train_batch_size * args.n_device,
...
def parse_arguments():
    ...
    parser.add_argument('--json-summary', type=str, default="logger.json",
...
def setup_training(args):
    ...
    print("device: {} n_device: {}, distributed training: {}, 16-bits training: {}".format(
        device, args.n_device, args.distributed, args.fp16))
...
def take_optimizer_step(args, optimizer, model, overflow_buf, global_step):
    ...
    if args.allreduce_post_accumulation:
        ...
        if had_overflow == 0:
            ...
            if is_main_process():
                scaler = _amp_state.loss_scalers[0]
                logger.info("loss_scale: {}".format(scaler.loss_scale()))
        ...
        if args.device.lower() == "gpu":
             ...
             dm.optimizer_step(optimizer, safe_async=not args.distributed, model=model)
        #optimizer.zero_grad()
        for param in model.parameters():
            param.grad = None
        global_step += 1
...
def main():
    ...
    if args.do_train:
        if is_main_process():
            ...
            logger.info("batch_size_per_device: {}".format(args.train_batch_size))
            ...
            for f_id in range(f_start_id + 1 , len(files)):
                ...
                for step, batch in enumerate(train_iter):
                    ...
                    if global_step >= args.steps_this_run or timeout_sent:
                        ...
                        if is_main_process():
                            logger.info("average_loss: {},step_loss: {},learning_rate: {}".format(average_loss / (args.log_freq * divisor),\
                              loss.item() * args.gradient_accumulation_steps / divisor, optimizer.param_groups[0]['lr']))
                        average_loss = 0
                    ...
                    if global_step >= args.steps_this_run or training_steps % (
                        ...
                        if is_main_process() and not args.skip_checkpoint:
                            ...
                            logger.info("checkpoint_step: {}".format(global_step))
...
if __name__ == "__main__":
    ...
    if is_main_process():
        ...
        training_perf = args.train_batch_size * args.gradient_accumulation_steps * device_count\
                        * (global_step - args.resume_step + skipped_steps) / train_time_raw
        logger.info("e2e_train_time: {},training_sequences_per_second: {},inal_loss: {},raw_train_time: {}".format(e2e_time,training_perf,final_loss,train_time_raw))

...
def all_gather_list(data, max_size=16384,device = None):
    ...
    if torch.cuda.is_available():
        ...
        in_buffer = torch.ByteTensor(max_size)
        out_buffers = [
                torch.ByteTensor(max_size).to(device)
                for i in range(world_size)
            ]
    ...
    if torch.cuda.is_available():
        dist.all_gather(out_buffers, in_buffer.cuda())
    else:
        dist.all_gather(out_buffers, in_buffer.to(device))

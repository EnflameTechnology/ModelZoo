...
class SequenceGenerator(object):
    ...
    def generate_batched_itr(
        ...
        cuda=False, timer=None, prefix_size=0,device_type="cpu",device=None
    ):
        ...
        for sample in data_itr:
            if device_type == "cuda":
                ...
                s = utils.move_to_device(sample,device)
            ...
            if step < maxlen:
                if prefix_tokens is not None and step < prefix_tokens.size(1):
                    ...
                    if self.sampling_topk > 0:
                        ...
                        exp_probs = values.true_divide_(self.sampling_temperature).exp()
                        if step == 0:
                        ...
                        exp_probs = probs.true_divide_(self.sampling_temperature).exp_().view(-1, self.vocab_size)
        ...
        for model, encoder_out in zip(self.models, encoder_outs):
            ...
            if attn is not None:
                if avg_attn is None:
                    ...
                    avg_attn.add_(attn)
        avg_probs.true_divide_(len(self.models))
        avg_probs.log_()
        if avg_attn is not None:
            avg_attn.true_divide_(len(self.models))

...
def collate_tokens_static(values, pad_idx, eos_idx, left_pad, move_eos_to_beginning=False, pad_sequence=1,sentence_length=128):
    """Convert a list of 1d tensors into a padded 2d tensor."""
...
def collate(samples, pad_idx, eos_idx, left_pad_source=True, left_pad_target=False, pad_sequence=1,static_batch=False,max_length=128):
    if len(samples) == 0:
        return {}

    def merge(key, left_pad, move_eos_to_beginning=False):
        if static_batch:
            return collate_tokens_static(
            [s[key] for s in samples],
            pad_idx, eos_idx, left_pad, move_eos_to_beginning,
            pad_sequence,max_length)
...
class EpochBatchIterator(object):
    ...
    def __init__(
        ...
        num_shards=1, shard_id=0, epoch=0,batch_size=24,seq_length=128,static_batch=False,test_data=False
    ):
        ...
        with numpy_seed(self.seed):
            if not self.static_batch:
                ...
                self.frozen_batches = tuple(self._batch_generator_static())
    def __len__(self):
    ...
    def _batch_generator_static(self):
            src_dict = deepcopy(self.dataset.src_dict)
            batch = []
            def is_batch_full():
                if len(batch) == 0:
                    return False
            ...
            for idx in self.dataset.ordered_indices(self.seed, self.epoch):
                ...
                batch.append(idx)
            self.ignore_index = ignored
            if self.test_data or len(batch) == self.max_sentences:
                yield batch
            else:
                self.ignore_index.extend(batch)

            if len(self.ignore_index) > 0:
                print((
                    '| WARNING: {} samples have invalid sizes and will be skipped, '
                    'max_positions={}, first few sample ids={}'
                ).format(len(self.ignore_index), self.max_positions, self.ignore_index[:10]))

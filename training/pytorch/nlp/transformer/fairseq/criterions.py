...
class LabelSmoothedCrossEntropyCriterion(_Loss):
    ...
    def forward(self, norm_probs, target, reduce=True):
        ...
        #if reduce:
        #    nll_loss = (nll_loss * non_pad_mask.int()).sum()
        #    smooth_loss = (smooth_loss * non_pad_mask.int()).sum()
        #eps_i = self.eps / lprobs.size(-1)
        #loss = (1. - self.eps) * nll_loss + eps_i * smooth_loss

        target = target.view(-1)
        lprobs = norm_probs.view(-1, norm_probs.size(-1))
        nll_loss = F.nll_loss(lprobs, target, size_average=False, ignore_index=self.padding_idx,
                              reduce=reduce)
        smooth_loss = -lprobs.sum(dim=-1, keepdim=True)
        if reduce:
            target = target.view(-1,1)
            non_pad_mask = target.ne(self.padding_idx).int()

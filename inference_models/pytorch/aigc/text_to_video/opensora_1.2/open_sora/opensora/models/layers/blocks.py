...
from open_sora.opensora.acceleration.communications import all_to_all, split_forward_gather_backward
...
class Attention(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        qkv = qkv.view(qkv_shape).permute(2, 0, 3, 1, 4).contiguous() # not add, then gcu is slower
        q, k, v = qkv.unbind(0)
        ...
        if False:
            ...
            x = torch.nn.functional.scaled_dot_product_attention(
                q, k, v,
                dropout_p=self.attn_drop.p if self.training else 0.0,
                scale=self.scale).permute(0, 2, 1, 3).contiguous()
        x_output_shape = (B, N, C)
        if not enable_flash_attn and False:
            x = x.transpose(1, 2)

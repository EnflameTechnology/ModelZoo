...
from open_sora.opensora.registry import MODELS, build_module
from open_sora.opensora.utils.ckpt_utils import load_checkpoint
...
class VideoAutoencoderPipeline(PreTrainedModel):
    ...
    def __init__(self, config: VideoAutoencoderPipelineConfig):
        ...
        self.spatial_vae = VideoAutoencoderKL(
            from_pretrained=config.vae_2d["from_pretrained"],
            micro_batch_size=config.vae_2d["micro_batch_size"],
            cache_dir=None,
            local_files_only=config.vae_2d["local_files_only"],
            subfolder=config.vae_2d["subfolder"])
        self.temporal_vae = VAE_Temporal_SD(from_pretrained=config.vae_temporal["from_pretrained"]) #build_module(config.vae_temporal, MODELS)
...
def OpenSoraVAE_V1_2(
    micro_batch_size=4,
    micro_frame_size=17,
    from_pretrained_vae2d=None,
    from_pretrained_vae3d=None,
    ...
    vae_2d = dict(
        ...
        from_pretrained=from_pretrained_vae2d,
        subfolder="vae",
    ...
    if from_pretrained_vae3d is not None: # and not os.path.isdir(from_pretrained):
        model = VideoAutoencoderPipeline.from_pretrained(from_pretrained_vae3d, **kwargs)
    else:
        config = VideoAutoencoderPipelineConfig(**kwargs)
        model = VideoAutoencoderPipeline(config)

        if from_pretrained_vae3d:
            load_checkpoint(model, from_pretrained_vae3d)
    return model

...
from torch.cuda.amp import autocast  ### Extra added
...
from tops_models.model_compare_hook import register_compare
...
def train_one_epoch(model, criterion, optimizer, data_loader, device, epoch, lr_scheduler, tb_writer, scaler):
    ...
    for image, target in data_loader:
        ...
        if args.amp:
            with autocast(enabled=True):  ### Extra added
                output = model(image)  ### Extra added
                loss = criterion(output, target)  ### Extra added
            scaler.scale(loss).backward() ### Extra added
        else:
            output = model(image)
        ...
        #     loss.backward()
        if args.device=='gcu':
            jit_mode=torch_gcu.JitRunMode.ASYNC if args.distributed else torch_gcu.JitRunMode.SAFEASYNC
            if args.enable_horovod:
                ...
                torch_gcu.optimizer_step(optimizer, [loss, output], model=model, mode=jit_mode, device=device)
            else:
                if args.amp:
                    scaler.step(optimizer)
                    scaler.update()
                    torch_gcu.unlazy(torch_gcu.collect_amp_training_params(model, optimizer, scaler) + [loss, output], mode=jit_mode)
                else:
                    torch_gcu.optimizer_step(optimizer, [loss, output], mode=jit_mode, model=model)
        else:
            if args.amp:
                scaler.step(optimizer)  ### Extra added
                scaler.update()  ### Extra added
            else:
                optimizer.step()
        local_step += 1
...
def main(args):
    ...
    if args.lr_schedule == "step":
        ...
        lr_policy = lr_polynomial_policy(initial_learning_rate, args.warmup, args.decay_steps, args.end_lr, args.power, args.cycle)
    else:
        raise Exception('{} lr_schedule is not supported.'.format(args.lr_schedule))
    ...
    if args.enable_horovod:
        ...
        for name, param in params:
            if "num_batches_tracked" in name or "running_mean" in name or "running_var" in name:
                print("skip: broadcast name: {}".format(name))
                continue
            hvd.broadcast_(param, root_rank=0)
    ...
    if args.resume:
        ...
        for k, v in checkpoint['model'].items():
            if "module" in k:
                name = k[7:] # delete the prefix module.
        ...
        ## resume epoch and train_global_step
        args.start_epoch = checkpoint['epoch'] + 1
        var.train_global_step = checkpoint['train_global_step'] + 1
        logger.info("load checkpoint from {} done. ".format(args.resume))

    if args.distributed:
        if args.device == 'gcu':
            model = torch_gcu.nn.parallel.DistributedDataParallel(model, device_ids=[args.local_rank], broadcast_buffers=args.broadcast_buffers)
        else:
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if (epoch + 1) % args.eval_freq == 0:
            ...
            logger.info(
                    "[Model_Eval] epoch: {}; global_step: {}; eval_acc1_epoch: {}; eval_acc5_epoch: {}".format(
                        epoch,
                        var.train_global_step,
                        eval_accuracy_epoch,
                        eval_accuracy_5_epoch))
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        metavar="SCHEDULE",
        choices=["step", "linear", "cosine" , "exponential", "polynomial"],
    ...
    parser.add_argument("--decay-steps", default=10, type=int, help="set decay steps use polynomial lr-schedule")
    parser.add_argument("--power", default=2, type=float, help="set power when use polynomial lr-schedule")
    parser.add_argument("--cycle", default=False, type=bool, help="set cycle when use polynomial lr-schedule")
    parser.add_argument(
        "--cache-dataset",
        dest="cache_dataset",
    ...
    parser.add_argument('--amp_eval', action='store_true',
                        help='Use amp for mixed precision evaluate')
...
if __name__ == "__main__":
    ...
    # GCU_UMD_FLAGS="cqm_executor_enable=true cluster_as_device=true" which
    ...
    logger = tops_logger()
    if args.enable_horovod:
        tb_writer = SummaryWriter('runs/tensorboard/dtu_{}'.format(hvd.rank()))
    else:
        tb_writer = SummaryWriter('runs/tensorboard/dtu_{}'.format(args.local_rank))
    var = global_var()
    timers = Timers()

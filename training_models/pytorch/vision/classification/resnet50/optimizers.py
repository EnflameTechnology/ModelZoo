...
def lr_polynomial_policy(base_lr, warmup_length, decay_step, end_lr=0.0001, power=2.0, cycle=False):
    def _lr_fn(iteration, epoch):
        ...
        if epoch < warmup_length:
            ...
            if cycle==False:
                global_step = min(iteration, decay_steps)
                lr = (base_lr - end_lr)*(1 - global_step / decay_steps)**power + end_lr
            else:

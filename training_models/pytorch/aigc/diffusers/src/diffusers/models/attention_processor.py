...
from .utils import is_torch_dtu_available
...
class Attention(nn.Module):
    ...
    def get_attention_scores(self, query, key, attention_mask=None):
        ...
        if attention_mask is None:
            ...
            attention_scores = attention_mask + self.scale * torch.bmm(query, key.transpose(-1, -2))
...
class AttnProcessor2_0:
    ...
    def __init__(self):
        if is_torch_dtu_available():
            if not hasattr(F, "scaled_dot_product_attention_continuous"):
    ...
    def __call__(
        ...
        if (hidden_states.shape[1] == 9216 and encoder_hidden_states.shape[1] == 9216) \
            or (hidden_states.shape[1] == 2304 and encoder_hidden_states.shape[1] == 2304) \
            or (hidden_states.shape[1] == 576 and encoder_hidden_states.shape[1] == 576) \
            or (hidden_states.shape[1] == 4096 and encoder_hidden_states.shape[1] == 4096):
            # or (hidden_states.shape[1] == 1024 and encoder_hidden_states.shape[1] == 1024):
            ...
            if is_torch_dtu_available():
                ...
                # the output of sdp = (batch, num_heads, seq_len, head_dim)
                # TODO: add support for attn.scale when we move to Torch 2.1
                hidden_states = F.scaled_dot_product_attention_continuous(
                    query, key, value, attn_mask=attention_mask, dropout_p=0.0, is_causal=False
                )

...
class Accelerator:
    ...
    **Available attributes:**
    ...
    def __init__(
        ...
        if self.state.mixed_precision == "fp16" and self.distributed_type == DistributedType.NO and is_gcu_available():
            self.native_amp = True
            kwargs = self.scaler_handler.to_kwargs() if self.scaler_handler is not None else {}
            self.scaler = torch_gcu.amp.GradScaler(use_zero_grad=True, **kwargs) # autocast support #TODO double check amp autocast
        elif (
            self.state.mixed_precision == "fp16"
            and self.device.type != "cpu"
            and self.distributed_type not in (DistributedType.DEEPSPEED, DistributedType.MEGATRON_LM, DistributedType.MULTI_GCU)
        ):
            self.native_amp = True
        ...
        elif self.state.mixed_precision == "fp16" and self.distributed_type in (DistributedType.MULTI_GCU): #gcu fp16 amp training
            self.native_amp = True
            kwargs = self.scaler_handler.to_kwargs() if self.scaler_handler is not None else {}
            self.scaler = torch_gcu.amp.GradScaler(use_zero_grad=True, **kwargs) # autocast support #TODO double check amp autocast
    ...
    def prepare_model(self, model: torch.nn.Module, device_placement=None):
        ...
        if self.distributed_type == DistributedType.MULTI_GPU:
            ...
            if any(p.requires_grad for p in model.parameters()):
                kwargs = self.ddp_handler.to_kwargs() if self.ddp_handler is not None else {}
                model = torch_gcu.nn.parallel.DistributedDataParallel(model, device_ids=[self.local_process_index])
        ...
        if self.native_amp:
            if is_gcu_available():
                model.forward = torch_gcu.amp.autocast(enabled=True)(model.forward)
            else:
    ...
    def prepare_data_loader(self, data_loader: torch.utils.data.DataLoader, device_placement=None):
        ...
        if is_gcu_available(): # on GCU, data preparation, we fix only the sampler
            return data_loader
    ...
    def prepare_optimizer(self, optimizer: torch.optim.Optimizer, device_placement=None):
        ...
        if is_gcu_available(): #GCU, optimizer device placement is not needed
    ...
    def clip_grad_norm_(self, parameters, max_norm, norm_type=2):
        ...
        self.unscale_gradients()
        if is_gcu_available():
            from .gcu_clip_grad_norm import clip_grad_norm_

...
from .imports import is_torch_distributed_available, is_tpu_available, is_gcu_available
...
if is_gcu_available(check_device=False):
    import torch_gcu
...
def _gcu_gather(tensor):
    "gcu gather op support"
    def _gcu_gather_one(tensor):
        if tensor.ndim == 0:
            tensor = tensor.clone()[None]
        output_tensors = [tensor.clone() for _ in range(torch_gcu.distributed.get_world_size())]
        torch_gcu.distributed.all_gather(output_tensors, tensor)
        return torch.cat(output_tensors, dim=0)

    return recursively_apply(_gcu_gather_one, tensor, error_on_other_type=True)
...
def gather(tensor):
    ...
    elif PartialState().distributed_type == DistributedType.MULTI_GCU:
        return _gcu_gather(tensor)
...
def _gcu_broadcast(data, src=0):
    def _gcu_broadcast_one(tensor, src=0):
        torch_gcu.distributed.broadcast(tensor, src=src)
        return tensor

    return recursively_apply(_gcu_broadcast_one, data, error_on_other_type=True, src=src)
...
def broadcast(tensor, from_process: int = 0):
    ...
    if PartialState().distributed_type == DistributedType.TPU:
        ...
        return _gcu_broadcast(tensor, src=from_process)
    elif PartialState().distributed_type == DistributedType.MULTI_CPU:
        return _gpu_broadcast(tensor, src=from_process)
    else:
        return tensor
...
def reduce(tensor, reduction="mean"):
    ...
    def _reduce_across_processes(tensor, reduction="mean"):
        ...
        elif state.distributed_type == DistributedType.MULTI_GCU:
            torch_gcu.distributed.all_reduce(cloned_tensor, ReduceOp.SUM)

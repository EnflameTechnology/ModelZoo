...
def launch_command_parser(subparsers=None):
    ...
    hardware_args.add_argument(
            "--gcu", default=False, action="store_true", help="Whether or not this should launch a GCU training"
            )
    hardware_args.add_argument(
            "--multi_gcu",default=False,action="store_true",
            help="Whether or not this should launch a distributed GCU training.",
    )
    ...
    distributed_args.add_argument(
             "--gcu_ids",default=None,
             help="What GCUs (by id) should be used for training on this machine as a comma-seperated list",)
    distributed_args.add_argument(
        "--same_network",
        default=False,
        action="store_true",
...
def multi_gcu_launcher(args):
    if is_torch_version(">=", "1.9.1"):
        import torch.distributed.run as distrib_run
    else:
        raise NotImplementedError("Native multi-GPU training through `accelerate launch` requires pytorch>=1.9.1. ""Please call `torch.distributed.launch` directly instead.")
    ...
    if num_machines > 1:
        ...
        if getattr(args, "same_network", False):
            ...
            setattr(args, "master_port", int(main_process_port))
        else:
    ...
    current_env = os.environ.copy()
    gcu_ids = getattr(args, "gcu_ids", "all")
    if gcu_ids != "all" and args.gcu_ids is not None:
        current_env["TOPS_VISIBLE_DEVICES"] = gcu_ids
    args = _filter_args(
            args,distrib_run.get_args_parser(),
            ["--training_script", args.training_script, "--training_script_args", args.training_script_args],)
    if num_machines == 1:
        setattr(args, "standalone", True)
    setattr(args, "use_env", True)
...
def launch_command(args):
    ...
    if args.use_deepspeed and not args.cpu:
        ...
        multi_gpu_launcher(args)
    elif args.gcu and not args.cpu:
        simple_launcher(args)
    elif args.multi_gcu and not args.cpu:
        multi_gcu_launcher(args)

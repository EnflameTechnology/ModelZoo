...
from .utils import DistributedType, honor_type, is_torch_version, is_tpu_available, is_gcu_available
...
class AcceleratedOptimizer(torch.optim.Optimizer):
    ...
    def step(self, closure=None, model=None, unlazy_list=None):
        if self.gradient_state.sync_gradients:
            if self.accelerator_state.distributed_type == DistributedType.TPU:
                optimizer_args = {"closure": closure} if closure is not None else {}
                xm.optimizer_step(self.optimizer, optimizer_args=optimizer_args)
            elif self.scaler is not None and not is_gcu_available():
            ...
            elif self.scaler is None and is_gcu_available(): #GCU amp fp16 training, to be the same with tpu training
                distributed = self.accelerator_state.distributed_type == DistributedType.MULTI_GCU
                torch_gcu.optimizer_step(self.optimizer, unlazy_list, mode=torch_gcu.JitRunMode.ASYNC if distributed else torch_gcu.JitRunMode.SAFEASYNC)
            elif self.scaler is not None and is_gcu_available():
                self.scaler.step(self.optimizer)
                self.scaler.update()
                if model is None or unlazy_list is None:
                    raise ValueError("torch_gcu training, model and unlazy_list should be send to step")
                torch_gcu.unlazy(torch_gcu.collect_amp_training_params(model, self.optimizer, self.scaler) + unlazy_list)
            else:

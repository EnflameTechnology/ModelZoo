    ...
    **Available attributes:**
    ...
    def __init__(self, cpu: bool = False, **kwargs):
        ...
        if not self.initialized:
            ...
            if (
            ...
            elif is_gcu_available() and int(os.environ.get("LOCAL_RANK", -1)) != -1 and not cpu:
                self.distributed_type = DistributedType.MULTI_GCU
                #if not torch.distributed.is_initialized():
                #    torch_gcu.distributed.init_process_group(backend="eccl")

                self.num_processes = int(os.environ['WORLD_SIZE'])
                self.process_index = int(os.environ["RANK"])
                self.local_process_index = int(os.environ.get("LOCAL_RANK", -1)) # TODO, double check the local rank on gcu is 0 or 1
                if not torch.distributed.is_initialized():
                    # torch_gcu.distributed.init_process_group(backend="eccl", world_size=self.num_processes, rank=self.local_process_index)
                    torch_gcu.distributed.init_process_group(backend="eccl", init_method='env://', world_size=self.num_processes, rank=self.process_index)
                self.device = torch_gcu.gcu_device(self.local_process_index) # set gcu device
                self.backend = "eccl"
                ...
                if self.device is None:
                    if cpu or not (torch.cuda.is_available() or is_mps_available() or is_gcu_available()):
                        self.device = torch.device("cpu")
                    elif is_mps_available():
                        os.environ["PYTORCH_ENABLE_MPS_FALLBACK"] = "1"
                        self.device = torch.device("mps")
                    elif is_gcu_available():
                        self.device = torch_gcu.gcu_device(self.local_process_index) # for single GCU card
                    else:
    ...
    def wait_for_everyone(self):
        ...
        elif self.distributed_type == DistributedType.MULTI_GCU:
            torch_gcu.distributed.barrier()

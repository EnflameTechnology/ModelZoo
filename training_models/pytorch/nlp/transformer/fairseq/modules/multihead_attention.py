...
class KeyValueLinears(torch.autograd.Function) :
    ...
    def backward(ctx, k_grad, v_grad) :
        ...
        v = torch.addmm(k, v_grad.view(v_grad.size(0) * v_grad.size(1),
                                       v_grad.size(2)), weights_v.transpose(0, 1), beta=1.0)
...
class SelfAttentionLinears(torch.autograd.Function):
    ...
    def forward(ctx, input, weights_q, weights_k, weights_v, scale) :
        ...
        v = torch.addmm(
            input.view(input.size(0)*input.size(1),input.size(2)),
            input.view(input.size(0) * input.size(1),
            input.size(2)), weights_v, beta=0.0, alpha=1.0)
        v=v.view(input.size(0), input.size(1), input.size(2))
    ...
    def backward(ctx, q_grad, k_grad, v_grad) :
        ...
        k = torch.addmm(q, k_grad.view(k_grad.size(0) * k_grad.size(1),
                                k_grad.size(2)), weights_k.transpose(0, 1), beta=1.0)
        k_grad = k_grad.view(k_grad.size(0)*k_grad.size(1), k_grad.size(2))
        weights_k_grad = torch.mm(input, k_grad)
        # v = k.addmm_(v_grad.view(v_grad.size(0) * v_grad.size(1), v_grad.size(2)), weights_v.transpose(0,1), beta=1.0)
        v = torch.addmm(k, v_grad.view(v_grad.size(0) * v_grad.size(1),
...
class StridedBmm1Func(torch.autograd.Function):
    ...
    def backward(ctx, grad_output):
        ...
        grad_input2 = torch.bmm(grad_output.transpose(1, 2),
...
def key_value_linears(input: Tensor, weights_k: Tensor, weights_v: Tensor):
    if not torch.jit.is_scripting():
        ...
        v = torch.einsum('ij,jk->ik',
        input.view(input.size(0) * input.size(1), -1),
        weights_v)
        v = v.view(input.shape)
        return k, v

def self_attn_linears(input: Tensor, weights_q: Tensor, weights_k: Tensor,
                      weights_v: Tensor, scale: float):
    if not torch.jit.is_scripting():
        return SelfAttentionLinears.apply(input, weights_q, weights_k,
         weights_v, scale)
    else:
        q = scale * torch.einsum('ij,jk->ik',
                                 input.view(input.size(0)*input.size(1), -1),
...
class QueryLinearModule(torch.nn.Module):
    def __init__(self, scale):
        super(QueryLinearModule, self).__init__()
        self.scale = scale

    def forward(self, x):
        input, weights = x
        output = self.scale * torch.matmul(input.view(input.size(0) * input.size(1), input.size(2)), weights)
        return output


class KeyValueLinearsModule(torch.nn.Module) :
    def __init__(self):
        super(KeyValueLinearsModule, self).__init__()

    def forward(self, x):
        input, weights_k, weights_v = x
        k = torch.matmul(input.view(input.size(0) * input.size(1), input.size(2)), weights_k)
        k = k.view(input.size(0), input.size(1), input.size(2))
        v = torch.matmul(input.view(input.size(0) * input.size(1), input.size(2)), weights_v)
        v = v.view(input.size(0), input.size(1), input.size(2))
        return k, v


class SelfAttentionLinearsModule(torch.nn.Module):
    def __init__(self,scale):
        super(SelfAttentionLinearsModule, self).__init__()
        self.scale = scale

    def forward(self, x) :
        input, weights_q, weights_k, weights_v = x
        q = self.scale*torch.matmul(input.view(input.size(0) * input.size(1), input.size(2)), weights_q)
        q = q.view(input.size(0), input.size(1), input.size(2))
        k = torch.matmul( input.view(input.size(0) * input.size(1), input.size(2)), weights_k)
        k = k.view(input.size(0), input.size(1), input.size(2))
        v = torch.matmul( input.view(input.size(0) * input.size(1), input.size(2)), weights_v)
...
class StridedBmm1FuncModule(torch.nn.Module):
    def __init__(self):
        super(StridedBmm1FuncModule,self).__init__()
...
class StridedBmm2FuncModule(torch.nn.Module):
    def __init__(self):
        super(StridedBmm2FuncModule,self).__init__()
...
#            self.register_parameter('in_proj_bias', None)
        ...
        self.query_linear = QueryLinearModule(self.scaling)
        self.key_value_linears = KeyValueLinearsModule()
        self.self_attn_linears = SelfAttentionLinearsModule(self.scaling)
        self.strided_bmm1 = StridedBmm1FuncModule()
        self.strided_bmm2 = StridedBmm2FuncModule()
...
#            nn.init.constant_(self.in_proj_bias, 0.)
    ...
    def forward(self, query: Tensor, key: Tensor, value: Tensor,
        ...
        if torch.cuda.is_available():
            ...
            qv_same = query is value
            qkv_same = qv_same and kv_same
        ...
        if qkv_same:
            # self-attention
            q, k, v = self.self_attn_linears((query, self.in_proj_weight_q,
            self.in_proj_weight_k,
            self.in_proj_weight_v))
        elif kv_same:
            # encoder-decoder attention
            q = self.query_linear((query,self.in_proj_weight_q))
            if not(saved_state is not None and 'prev_key' in saved_state and static_kv):
                k, v = self.key_value_linears((key ,self.in_proj_weight_k, self.in_proj_weight_v))
        else:
            q = torch.addmm(
        ...
        if saved_state is not None:
            if 'prev_key' in saved_state:
                k = torch.cat((saved_state['prev_key'], k), dim=1)
            if 'prev_value' in saved_state:
                v = torch.cat((saved_state['prev_value'], v), dim=1)
        ...
        q = q.contiguous().view(bsz, tgt_len, self.num_heads,
                                self.head_dim).permute(0, 2, 1, 3).contiguous().view(
                                    bsz * self.num_heads, tgt_len,
                                    self.head_dim)
        k = k.contiguous().view(bsz, src_len, self.num_heads,
                                self.head_dim).permute(0, 2, 3, 1).contiguous().view(
                                    bsz * self.num_heads, self.head_dim, src_len)
        v = v.contiguous().view(bsz, src_len, self.num_heads,
                                self.head_dim).permute(0, 2, 1, 3).contiguous().view(
                                    bsz * self.num_heads, src_len,
                                    self.head_dim)

        attn_weights = self.strided_bmm1((q, k))
        assert list(
        ...
        if need_weights:
            ...
            attn_weights = attn_weights.new_empty(0)
    ...
    def buffered_mask(self, tensor):
        ...
        if self._mask.size(0) < dim:
            self._mask = torch.triu(
                utils.fill_with_neg_inf(torch.zeros(dim, dim)).to(tensor.device), 1).to(tensor.device)
